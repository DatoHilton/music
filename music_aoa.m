function music_aoa(csi_data, params)
...
H = squeeze(csi(:, 11, :));  % 取第11个子载波，H为3×n
...
En = EV(:, params.N_source+1: params.N_Rx_antenna);  % 取特征向量矩阵的第N_source+1到N_Rx_antenna列特征向量组成噪声子空间
...
for i = 1:361
    ...
    theta_m = angle(i) * params.ang2rad;  % 转为弧度
    a = exp(-1i * 2 * pi * (params.frequency/params.c) * params.pos_antenna * sin(theta_m)).';
